...
function make_menu_tree({q, tb_role, tb_menu, role_user, role_menu,}) {
    const userRoles = role_user.filter(ru => ru.username === q.username).map(ru => ru.role);
    ...
    const menu_tree = buildMenuTree(fullMenus)
