...
//调用方式 const menuTree = build_menus_tree({menus:menus_flat});
export default function build_menus_tree({arr, key_id = "depart_id", key_parent = 'parent_id', parentId = 0}) {
    arr = JSON.parse(JSON.stringify(arr))
    const menu_tree: any = []
    for (const o of arr) {
        if (o[key_parent] === parentId) {
            const children = build_menus_tree({arr, key_id: key_id, key_parent: key_parent, parentId: o[key_id]});
