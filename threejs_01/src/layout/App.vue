        ...
        <el-container>
            ...
            <el-aside width="180px">
                <el-menu :default-active="$route.path" background-color="#24292E" active-text-color="#ffd04b"
                    ...
                    <template
                        v-for="ele in $router.options.routes.filter(o => o.name && !o?.kind?.includes('bottom')) ">
                        <el-menu-item class="没有子菜单" v-if="!ele.children || ele?.children?.length === 0"
                            :index="ele.path_full || ele.path">
...
export default {
    ...
    async mounted() {
        ...
        console.log('App.vue mounted', this.$router.options.routes)
