<template>
  ...
  <el-dialog v-model="tree_permiss.show" width="600px" height="600px" title="权限-编辑-职位" draggable>
    <div style="display: flex; gap: 20px;">
      ...
      <el-button type="primary" @click="save_permiss_menu_position()">保存</el-button>
    </div>
  ...
  data() {
    return {
      ...
      menu_original: [
        ...
        { is_depart: false, title: "修改-职位-权限" },
        { is_depart: false, title: "删除-职位" },
  ...
  methods: {
    ...
    async menu_opt_click(item) {
      ...
      //组织:删除-组织
        ...
        await api.delete_depart({ id: this.tree_depart.node.id })
        await this.find_permiss_depart_position_tree()
      }
      // 职位=================================================================
      //职位:修改-职位-权限
      else if (!item.is_depart && item.title === "修改-职位-权限") {
        this.tree_permiss.show = true
        await this.find_permiss_depart_position_tree()
      }
      // 职位:删除-职位
      else if (!item.is_depart && item.title === "删除-职位") {
        let confirm = await ElMessageBox.confirm('确定删除吗', '删除提示', { cancelButtonText: '取消', confirmButtonText: '删除' })
        if (confirm != 'confirm') return
        await api.delete_position({ id: this.tree_depart.node.id })
        await this.find_permiss_depart_position_tree()
      } else {
        alert("等待开发")
      }
    ...
    //保存_职位_权限_数据
    async save_permiss_menu_position() {
      console.error('111---tree_permiss.data:', JSON.parse(JSON.stringify(this.tree_permiss.data)))
      let position_id = this.tree_depart.node.id
      let res = await api.save_permiss_menu_position({ position_id, tree_data: this.tree_permiss.data })
