...
async function make_data() {
    ...
    let role_temp = []
    await db.tb_depart.create({data: {id: 1, name: "大宇三维打印", parent_id: 0, tb_role: {create: role_temp}}})
    // await db.tb_depart.create({data: {id: 2, name: "客户", parent_id: 1, tb_role: {create: position}}})
    // await db.tb_depart.create({data: {id: 3, name: "内部人员", parent_id: 1, tb_role: {create: position}}})
    await db.tb_depart.create({data: {id: 4, name: "技术部", parent_id: 1, tb_role: {create: role_name}}})

    await db.tb_depart.create({data: {id: 5, name: "泉州分公司", parent_id: 1, tb_role: {create: role_temp}}})
    await db.tb_depart.create({data: {id: 6, name: "财务部", parent_id: 5, tb_role: {create: role_name}}})
    await db.tb_depart.create({data: {id: 7, name: "业务部", parent_id: 5, tb_role: {create: role_name}}})

    // await db.tb_depart.create({data: {id: 8, name: "德化分公司", parent_id: 1, tb_role: {create: position}}})
    // await db.tb_depart.create({data: {id: 9, name: "财务部", parent_id: 8, tb_role: {create: position}}})
    // await db.tb_depart.create({data: {id: 10, name: "业务部", parent_id: 8, tb_role: {create: position}}})
    //
    // await db.tb_depart.create({data: {id: 11, name: "厦门分公司", parent_id: 1, tb_role: {create: position}}})
    // await db.tb_depart.create({data: {id: 12, name: "财务部", parent_id: 11, tb_role: {create: position}}})
    // await db.tb_depart.create({data: {id: 13, name: "业务部", parent_id: 11, tb_role: {create: position}}})

    // 权限表
    let role_permiss = [
        {menu_id: 0, role_id: 0, create: false, delete: false, update: false, find: false, view: false,},
    ]
    ////关联-职位权限



    await db.role_permiss.createMany({data: role_permiss})
