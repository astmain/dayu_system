...
export class user {
    ...
    async update_user(@Query() form) {
        ...
        await db.role_user.deleteMany({where: {user_id: form.user_id}})
        for (let i = 0; i < form.role_ids.length; i++) {
            let role_id = form.role_ids[i]
            await db.role_user.create({data: {user_id: user.id, role_id: role_id || 0}})
        }
        return tool.R.ok({msg: "成功/create_user", result: {}})
    }
    ...
    async delete_user(@Query() form) {
        ...
        await db.role_user.deleteMany({where: {id: form.user_id}})
