...
//  帮我封装成函数build_tree通用代码  可以让 tb_menu 和 tb_depart 都可以使用   他们的 menu_id 和 depart_id 是唯一的

function build_tree({ items, idKey, parentId = 0 }) {
    const tree = [];
    ...
    for (const child of children) {
        const node = {
            ...child,
            children: build_tree({ items, idKey, parentId: child[idKey] })
        };
        tree.push(node);
    }
...
console.log(JSON.stringify(build_tree({ items: tb_menu, key_id: 'menu_id' }), null, 2));
// console.log( JSON.stringify(build_tree({items:tb_depart,key_id: 'depart_id'}),null,2) );

// Example usage:
// const menuTree = build_tree(tb_menu, 'menu_id');
// const departTree = build_tree(tb_depart, 'depart_id');
